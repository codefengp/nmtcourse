...
public class AchievementEvaluationController {
    ...
    @PreAuthorize("@ss.hasPermission('nmt:achievement-evaluation:query')")
    public void exportReport(HttpServletResponse response,@RequestParam(name = "courseId") Long courseId, @RequestParam(name = "classId") Long classId) throws IOException {
        achievementEvaluationService.exportReport(response,courseId,classId);
    }
}