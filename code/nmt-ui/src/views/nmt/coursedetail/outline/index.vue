    ...
    <main class="content-main">
      <section id="s1" class="section-anchor">
        <el-card shadow="always" class="course-card">
          <template #header><span class="card-title">课程基本信息</span></template>
          <div class="info-grid">
            <div class="info-item">
              <span class="label">课程编码：</span><span class="value">{{ courseDetail.number }}</span>
            </div>
            <div class="info-item">
              <span class="label">课程名称：</span><span class="value">{{ courseDetail.name }}</span>
            </div>
            <div class="info-item">
              <span class="label">课程类别：</span><span class="value">{{ courseDetail.courseType }}</span>
            </div>
            <div class="info-item">
              <span class="label">课程性质：</span><span class="value">{{ courseDetail.courseProperty }}</span>
            </div>
            <div class="info-item">
              <span class="label">课程学时：</span><span class="value">{{ courseDetail.courseHour }}</span>
            </div>
            <div class="info-item">
              <span class="label">课程学分：</span><span class="value">{{ courseDetail.courseScore }}</span>
            </div>
            <div class="info-item">
              <span class="label">授课年级：</span><span class="value">{{ courseDetail.grade }}</span>
            </div>
            <div class="info-item">
              <span class="label">授课学期：</span><span class="value">{{ courseDetail.term }}</span>
            </div>
            <div class="info-item">
              <span class="label">适用专业：</span><span class="value">{{ courseDetail.majorType }}</span>
            </div>
            <div class="info-item">
              <span class="label">课程负责人：</span><span class="value">{{ courseDetail.teacherName || '—' }}</span>
            </div>
          </div>
        </el-card>
      </section>
        ...
        <el-card shadow="always" class="course-card">
          <template #header>
            <div class="card-header">
              ...
              <el-button type="primary" @click="addObjective">新增目标</el-button>
            </div>
          </template>
          <el-form ref="formRef" :model="objectForm" label-position="left">
            <el-row
            ...
            >
              <el-col :span="4">
                ...
                >
                  <el-input v-model="item.name" placeholder="名称" @blur="saveObjective(item)" />
                </el-form-item>
              </el-col>
              <el-col :span="13">
                <el-form-item
                ...
                >
                  <el-input v-model="item.content" placeholder="内容描述" @blur="saveObjective(item)" />
                </el-form-item>
              </el-col>
              <el-col :span="5">
                <el-form-item
              ...
              <el-col :span="2" class="text-right">
                <el-button type="danger" plain @click="removeObjective(item.id)">删除</el-button>
              </el-col>
            </el-row>
          </el-form>
        </el-card>
        ...
        <el-card shadow="always" class="course-card">
          <template #header>
            <div class="card-header">
              ...
              <el-button type="primary" @click="addMode">新增考核</el-button>
            </div>
          </template>
          <el-form ref="formRef" :model="modeForm" label-position="left">
            <el-row
            ...
            >
              <el-col :span="17">
                ...
                >
                  <el-input v-model="item.name" placeholder="请输入名称" @blur="saveMode(item)" />
                </el-form-item>
              </el-col>
              <el-col :span="5">
                <el-form-item
              ...
              <el-col :span="2" class="text-right">
                <el-button type="danger" plain @click="removeMode(item.id)">删除</el-button>
              </el-col>
            </el-row>
          </el-form>
        </el-card>
      ...
      <section id="s4" class="section-anchor">
        <el-card shadow="always" class="course-card">
          <template #header>
            <div class="card-header">
              ...
              <div v-if="tableData && tableData.length > 0">
                <el-button v-if="!editable" type="primary" @click="editable = true">编辑矩阵</el-button>
                <el-button v-else type="success" @click="saveObjectiveMode">保存矩阵</el-button>
              </div>
            </div>
          </template>
          <el-table
          ...
          >
            <el-table-column prop="objectiveName" label="课程目标" width="160" align="center" fixed />
...
const removeObjective = async (id: number) => {
  ElMessageBox.confirm('确认删除该课程目标吗？', '删除确认', {
...
const removeMode = async (id: number) => {
  ElMessageBox.confirm('确认删除该评价方式吗？', '删除确认', {
    confirmButtonText: '确认',
    cancelButtonText: '取消',
    type: 'warning'
  })
